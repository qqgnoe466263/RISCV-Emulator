...
static void decode_r_type_instr(rv_cpu_t *rv_cpu)
{
    ...
    rv_cpu->func3 = (rv_cpu->fetch_instr >> 12) & 0x7;
...
static void decode_rv32a_type_instr(rv_cpu_t *rv_cpu)
{
    rv_cpu->rd    = (rv_cpu->fetch_instr >> 7) & 0x1f;
    rv_cpu->func3 = (rv_cpu->fetch_instr >> 12) & 0x7;
    rv_cpu->rs1   = (rv_cpu->fetch_instr >> 15) & 0x1f;
    rv_cpu->rs2   = (rv_cpu->fetch_instr >> 20) & 0x1f;
    rv_cpu->rl    = (rv_cpu->fetch_instr >> 25) & 0x01;
    rv_cpu->aq    = (rv_cpu->fetch_instr >> 26) & 0x01;
    rv_cpu->func5 = (rv_cpu->fetch_instr >> 27) & 0x1f;
#if CONFIG_DBG
    DBG("[DEBUG] R-type (RV32A), rd = 0x%x, func3 = 0x%x, rs1 = 0x%x, rs2 = 0x%x, rl = 0x%x, aq = 0x%x, func5 = 0x%x \n",
                rv_cpu->rd, rv_cpu->func3, rv_cpu->rs1,
                rv_cpu->rs2, rv_cpu->rl, rv_cpu->aq, rv_cpu->func5);
#endif
...
#if CONFIG_DBG
    DBG("[DEBUG] I-type, op = 0x%x, rd = 0x%x, func3 = 0x%x, rs1 = 0x%x, imm = 0x%x\n",
                rv_cpu->opcode, rv_cpu->rd, rv_cpu->func3, rv_cpu->rs1, 
                rv_cpu->imm);
#endif
}
...
uint32_t decode_rv_instr(rv_cpu_t *rv_cpu)
{
    ...
    case 0x2f:
        decode_rv32a_type_instr(rv_cpu);
        break;
    default:
        printf("[PC : 0x%02x][Instr : 0x%08x]",
                rv_cpu->pc, rv_cpu->fetch_instr);
